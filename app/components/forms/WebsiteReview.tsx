...
const WebsiteReview = ({ planName }: WebsiteReviewProps) => {
    const { form, status, error, handleChange, handleSubmit } = useForm({
        initialValues: { name: '', email: '', phone: '', project: '', hosting: '', future_features: '' },
        formType: 'website-review',
        requiredFields: ['sender_name', 'sender_email', 'project', 'hosting', 'future_features'],
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                />
                <RadioGroup
                    label="Do you have hosting already?"
                    name="hosting"
                ...
                />
                <RadioGroup
                    label="Do you anticipate wanting to add more features or make improvements over time?"
                    name="future_features"
                    options={['No', 'Possibly', 'Yes']}
                    value={form.future_features}
                    onChange={handleChange}
                />
